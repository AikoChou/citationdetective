...
def extract(wikitext):
    sentences = []
    paragraphs = {}
    parser = citation_parser.CitationParser()
    ...
    for section in wikitext.get_sections(levels=[2], include_lead=True):
        ...
        for i, paragraph in enumerate(re.split("\n+", str(section))):
            parser.clear_sentences()
            try:
                parser.parse(paragraph)
            except:
                # One case caused by not-well-defined
                # reference tag. Need to look if there
                # is other reason cause this error.
                continue

            unsourced = parser.filter_unsourced()
            if unsourced:
                pid = mkid(section_name+str(i))
                paragraphs[pid] = paragraph
                for sent in unsourced:
                    if sent[0] in "|!<{":
                        # Not a sentence if it starts with
                        # special characters
                        continue
                    if len(word_tokenize(sent)) < cfg.sentence_min_word_count:
...
def parse(pageids):
    ...
    def insert(cursor, r):
        cursor.execute('''
            INSERT INTO statements (hashid, statement, context, section, rev_id, score)
            VALUES(%s, %s, %s, %s, %s, %s)
            ''', r)
    db = database.init_scratch_db()
    
    for i in range(0, len(pageids_list), 50):
        rows = []
        results = query_pageids(pageids_list[i: i + 50])
        for revid, title, wikitext in results:
            ...
            for text, pidx, section in sentences:
                hashid = mkid(title+section+text)
                row = [hashid, text, paragraphs[pidx], section, revid]
                rows.append(row)
